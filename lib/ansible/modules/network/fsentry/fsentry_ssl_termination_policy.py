...
DOCUMENTATION = '''
---
module: fsentry_ssl_termination_policy
version_added: "2.4"
short_description: Manage forum sentry SslTerminationPolicy(s).
description:
    - Create or delete a SslTerminationPolicy.
    - SslTerminationPolicy cannot be modified via the api and must be removed and recreated
options:
    associate_dn_to_user:
        description:
            - Should we associate the dn to a user in the forum?
        type: bool
    description:
        description:
    ...
    authenticate_client:
        description:
            - Should we authentication the user cert?
        type: bool 
...
notes:
    - SslTerminationPolicy cannot be modified via the api and must be removed and recreated using I(state=absent) and I(state=present)
'''
...
fsentry_ssl_termination_policy:
  name: hello_world
...
RETURN = '''
state:
  description: current state of the SslTerminationPolicy.
  returned: on success
  type: complex
  contains:
      status:
...
class FSentrySslTerminationPolicy(FSentryModuleBase):
    ...
    def __init__(self):
        ...
        self.associate_dn_to_user = None
        self.description = None
        self.signer_group = None
        self.key_pair = None
        self.acl_policy = None
        self.use_user_attr_only = None
        self.authenticate_client = None
        self.enabled_protocols = None        

          
                
        super(FSentrySslTerminationPolicy, self).__init__(self.module_arg_spec,
    ...
    def exec_module(self, **kwargs):         
        ...
        api = self.ssl_termination_policy_api
        results = dict()
        changed = False
        ...
        # We want the SslTerminationPolicy on the forum
        if self.state == 'present':
            
            #We only use want state if present. The model for SslTerminationPolicy doesnt except None for keypair.
            try:
                want_state = SslTerminationPolicy(name=self.name,

                                        associate_dn_to_user=self.associate_dn_to_user,
                                        description=self.description,
                                        signer_group=self.signer_group,
                                        key_pair=self.key_pair,
                                        enabled_protocols=self.enabled_protocols,
                                        acl_policy=self.acl_policy,
                                        use_user_attr_only=self.use_user_attr_only,
                                        authenticate_client=self.authenticate_client,
            ...
            if have_state is not None:
                ...
                if have_state == want_state:
                    ...
                    # get the props on this SslTerminationPolicy
        ...
        # We do not want the SslTerminationPolicy on the forum    
        ...
        if not self.check_mode:
            ...
            if self.state == 'present' and changed:
                # Create/Update the SslTerminationPolicy
                try:
                ...
                # delete the SslTerminationPolicy
                try:
