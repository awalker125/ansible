...
DOCUMENTATION = '''
---
module: fsentry_document
version_added: "2.4"
short_description: Manage forum sentry Document(s).
description:
    - Create, update or delete a Document.
    - Import/export a Document.
options:
    description:
        description:
            - Description to be added to the Document on the forum.
    document:
        description:
...
fsentry_document:
  ...
  document: '<?xml version="1.0" encoding="UTF-8"?><auth_token></auth_token>'
  ...
  contains:
      status:
          ...
          sample:
              ...
              - deleted
      document:
            description: the contents of the Document on the forum.
            returned: when state is present
            type: str
      description:
            description: the description of the Document on the forum.
            returned: when state is present
            type: str
      name:
            description: the name of the Document on the forum.
...
class FSentryDocument(FSentryModuleBase):
    ...
    def __init__(self):
        ...
        super(FSentryDocument, self).__init__(self.module_arg_spec,
    ...
    def exec_module(self, **kwargs):
        ...
        if self.state == 'present':
            ...
            if have_state is not None:
                ...
                if have_state == want_state:
                    ...
                    # get the props on this Document
        ...
        # We do not want the Document on the forum    
        ...
        #We want to do an import or export
            ...
            if self.src:
                ...
                # 3. If have_state is not None the Document exists on the forum so:
                ...
                if have_state is not None:
                    # the Document already exists. We will only import if force = true
                    if self.force:
                        changed = True
                    else:
                    ...
                    # the Document doesnt exist so we should run the import. There is no way to validate that the import contains the Document we are interested in unfortunately
                    changed = True    
            else:
                ...
                if have_state is None:
                    self.fail("cannot export none existent Document {0}".format(self.name))
            ...
            if self.state == 'present' and changed:
                ...
                if self.src:
                    ...
                    try:
                        ...
                        self.fail("Failed to export Document: {0}".format(e.message))
                    finally:
